...
public class MirrorStreamConsumerService extends DistcpBaseService{
  ...
  void doLocalCommit(Map<Path, Path> commitPaths) throws Exception{
    ...
    for (Map.Entry<Path, Path> entry : commitPaths.entrySet()) {
      ...
      destFs.mkdirs(entry.getValue().getParent());
      destFs.rename(entry.getKey(), entry.getValue());
    }
  }
  ...
  Map<Path, Path> prepareForCommit(Path tmpOut) throws Exception{
    ...
    */
    Path tmpStreamRoot = new Path(tmpOut.makeQualified(destFs).toString() + File.separator + getSrcCluster()
            .getUnqaulifiedFinalDestDirRoot());
    LOG.debug("tmpStreamRoot [" + tmpStreamRoot + "]");
    //tmpStreamRoot eg -   /databus/system/tmp/distcp_mirror_ua1_uj1/databus/streams/
    //multiple streams can get mirrored from the same cluster
    FileStatus[] fileStatuses = destFs.listStatus(tmpStreamRoot);
    ...
    if (fileStatuses != null) {
      for (FileStatus streamRoot : fileStatuses) {
        ...
        createListing(destFs, streamRoot, streamPaths);
  ...
  class PathComparator implements Comparator<FileStatus> {
    ...
    public int compare(FileStatus fileStatus, FileStatus fileStatus1) {
      /*Path eg: /databus/system/distcp_mirror_ua1_uj1/databus/streams/metric_billing/2012/1/13/15/7/
       * gs1104.grid.corp.inmobi.com-metric_billing-2012-01-16-07-21_00000.gz
       */
  ...
  private void createCommitPaths(Map<Path, Path> commitPaths, List<FileStatus> streamPaths) {
    //Path eg in streamPaths - /databus/system/distcp_mirror_ua1_uj1/databus/streams/metric_billing/2012/1/13/15/7
    // gs1104.grid.corp.inmobi.com-metric_billing-2012-01-16-07-21_00000.gz
    for (FileStatus fileStatus : streamPaths) {
      ...
      String finalPath = getDestCluster().getFinalDestDirRoot() + File.separator + streamName + File.separator
              + year + File.separator + month + File.separator + day + File.separator + hr + File.separator +
              min + File.separator + fileName;
