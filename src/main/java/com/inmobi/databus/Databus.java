...
public class Databus {
  ...
  public void start() throws Exception {
    ...
    for (Cluster c : config.getClusters().values()) {
      ...
      if (c.getName()
          .equalsIgnoreCase(config.getDestinationCluster().getName())) {
        logger.warn("Starting data consumer for Cluster[" + c.getName() + "]");
        copier = new DataConsumer(config);
      } else {
        logger.warn("Starting remote copier for cluster [" + c.getName() + "]");
