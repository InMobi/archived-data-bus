...
public class CopyMapper extends Mapper<Text, Text, Text, Text> {
	...
	public void map(Text key, Text value, Context context) throws IOException,
		...
		GzipCodec gzipCodec = (GzipCodec) ReflectionUtils.newInstance(GzipCodec.class, context.getConfiguration());
	...
	private Path getTempPath(Context context, Path src, String category,
		...
		Path tempPath = new Path(getTaskAttemptTmpDir(context),
						category + "-" + collector + "-" + src.getName()
										+ ".gz");
		return tempPath;
	}
	...
	public static Path getTaskAttemptTmpDir(Context context) {
		...
		return new Path(getJobTmpDir(context,
						attemptId.getJobID()), attemptId.toString());
	}
